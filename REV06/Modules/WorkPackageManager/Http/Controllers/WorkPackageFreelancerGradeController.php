...
class WorkPackageFreelancerGradeController extends Controller
{
    ...
    public function WorkPackageFreelancerGradeUpdate(Request $request, $id)
    {
        ...
        $FreelancerSectionID = FreelancerSection::where('users_id', $WorkPackage->winner_offer->freelancer->users_id)
            ...
            ->where('division_id', $WorkPackage->division_id)->first();
        ...
        if (auth()->user()->role === 'appraiser') {
            $GradeData['suggest_grade_message'] = $request->grade_message;
            $GradeData['suggest_grade_data'] = $gradesJson;
            $GradeData['suggest_grade'] = $average;
        } elseif (auth()->user()->role === 'chiefAppraiser' || auth()->user()->role === 'admin') {
            $GradeData['grade_message'] = $request->grade_message;
            $GradeData['grade_data'] = $gradesJson;
            $GradeData['grade'] = $average;
        }
        ...
        if ($WorkPackageFreelancerGrade) {
            $WorkPackageFreelancerGrade->update($GradeData);
        } else {
            $WorkPackageFreelancerGrade = WorkPackageFreelancerGrade::create($GradeData);
        }

        WorkPackageFreelancerGradeLog::create([
            'work_package_freelancer_grade_id' => $WorkPackageFreelancerGrade->id,
            'user_id' => auth()->id(),
            'freelancer_id' => $GradeData['freelancer_id'],
            'work_package_id' => $id,
