...
class WorkPackageTaskManagerAPIController extends Controller
{
    ...
    public
    function progressStore(WorkPackageProgressRequest $request, $id)
    {
        $Data['task_id'] = $id;
        $Data['user_id'] = auth('sanctum')->user()->id;
        $Data['status'] = 'new';
        if ($request->file('attachment')) {
            $Data['attachment'] = FileLibraryController::upload($request->file('attachment'), 'file', 'work-package/task/progress', 'task progress');
        }

        if (WorkPackageProgress::create($Data)) {
