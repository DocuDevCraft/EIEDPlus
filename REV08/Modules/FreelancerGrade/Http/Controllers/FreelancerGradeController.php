...
class FreelancerGradeController extends Controller
{
    ...
    {
        abort_unless(Gate::any(['isChiefAppraiser', 'isAppraiser']), 403);

        $grades = $request->input('grade_type', []);
        $messages = $request->input('grade_message_text', []);

        $weights = [1 => 1, 2 => 0.8, 3 => 0.9, 4 => 1, 5 => 0.8, 6 => 0.9, 7 => 1, 8 => 1, 9 => 0.7, 10 => 1];

        $totalWeighted = $totalWeights = 0;
        foreach ($grades as $k => $v) {
            $w = $weights[$k] ?? 1;
            $totalWeighted += floatval($v) * $w;
            $totalWeights += $w;
        }

        $average = $totalWeights ? round($totalWeighted / $totalWeights, 2) : 0;

        $section = FreelancerSection::findOrFail($id);

        $data = [];
        if (auth()->user()->role === 'appraiser') {
            $data = [
                'suggest_grade' => $average,
                'suggest_grade_data' => json_encode($grades),
                'suggest_grade_message' => json_encode($messages),
            ];
            $fromGrade = $section->suggest_grade;
        } else {
            $data = [
                'grade' => $average,
                'grade_data' => json_encode($grades),
                'grade_message' => json_encode($messages),
            ];
            $fromGrade = $section->grade;
        }

        $log = FreelancerGradeLog::create([
            'grade_data' => json_encode($grades),
            'grade_message' => json_encode($messages),
        ...
        $log->forceFill([
            'user_id' => auth()->id(),
            'freelancer_id' => $section->users_id,
