...
class WorkPackageManagerController extends Controller
{
    ...
    {
        Gate::authorize('isWorkPackageManager');

        $data = $request->validated();

        // فقط داده‌های کاربر
        $data['tag'] = $request->filled('tag') ? json_encode($request->tag) : null;
        ...
        // ⬅️ create فقط با فیلدهای غیرسیستمی
        $workPackage = WorkPackage::create($data);

        // ⬅️ مقداردهی فیلدهای سیستمی با forceFill
        $workPackage->forceFill([
            'uid' => auth()->id(),
            'status' => 'pending',
            'unique_id' => 'plus/' . uniqid(),
        ])->save();

        $freelancers = match ($request->work_package_type) {
            'hourly_contract' => $request->input('freelancer', []),
            'public' => $request->input('normal_freelancer', []),
            default => []
        };
        ...
        return redirect()
            ->route('work-package.edit', $workPackage->id)
            ->with('notification', [
                'class' => 'success',
    ...
    public function update(WorkPackageRequest $request, $id)
    {
        ...
        $canEdit =
            ($workPackage->status === 'pending' && $userRole === 'workPackageManager') ||
            (in_array($userRole, ['admin', 'sectionManager']) &&
                in_array($workPackage->status, ['pending', 'pre_accept', 'new']));
        ...
        foreach ([
                     'attachment_for_all' => 'work-package/attachment-for-all',
                     'attachment_for_winner' => 'work-package/attachment-for-winner',
                 ] as $field => $path) {
            if ($request->hasFile($field)) {
                $data[$field] = FileLibraryController::upload(
                    $request->file($field),
                    'file',
                    $path,
                    'work-package'
        ...
        $data['division_id'] = is_numeric($request->division_id)
        ...
        if (
            isset($data['offer_time_date']) &&
            !in_array($workPackage->status, ['pending', 'pre_accept'])
        ) {
            $from = Carbon::createFromTimestamp($workPackage->published_at);
            $to = Carbon::createFromTimestamp($data['offer_time_date']);
        ...
        /*
         * ✅ مقداردهی کنترل‌شده فیلدهای سیستمی
         */
        if ($request->filled('published_at')) {
            $workPackage->forceFill([
                'published_at' => Carbon::createFromTimestamp($request->published_at),
            ])->save();
        }
