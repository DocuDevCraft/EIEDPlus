...
class FreelancerOfferAPIController extends Controller
{
    ...
    public function storeOffer(FreelancerOfferRequest $request, $id)
    {
        ...
        if ($WorkPackage->work_package_type === 'public') {
            ...
            if ($signatureProcess) {
                $freelancerOffer = FreelancerOffer::where('work_package_id', $id)->where('user_id', auth()->user()->id)->where('status', 'pending')->where('status', '!=', 'rejected')->first();
                if ($freelancerOffer) {
                    if ($freelancerOffer = FreelancerOffer::find($freelancerOffer->id)->update([
                        'price' => $request->price,
                        'time' => $request->time,
                        'attachment' => $request->file('attachment') ? FileLibraryController::upload($request->file('attachment'), 'file', 'work-package/offer', 'WorkPackage Offer') : null,
                        'status' => 'pending',
                    ])) {
...
//            return response()->json(['status' => 'exist']);
                    ...
                    if ($freelancerOffer = FreelancerOffer::create([
