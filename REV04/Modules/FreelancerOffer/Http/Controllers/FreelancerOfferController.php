...
class FreelancerOfferController extends Controller
{
    ...
    {
        $WorkPackage = WorkPackage::select('id', 'offer_time', 'published_at')->find($id);
    ...
    {
        $WorkPackage = WorkPackage::select('id', 'title', 'offer_time', 'published_at')->find($id);
        $ShowPacket = Carbon::createFromFormat('Y-m-d H:i:s', $WorkPackage->published_at)->addDay($WorkPackage->offer_time)->format('Y-m-d H:i:s') <= Carbon::now()->format('Y-m-d H:i:s');

        if ($ShowPacket) {
            $engineeringManager = Users::where('role', 'engineeringManager')->select(['id', 'role', 'phone'])->first();
            $VerifyCode = rand(10000, 99999);
            $MessageText = 'درخواست دسترسی به لیست پیشنهادات بسته کاری "' . $WorkPackage->title . '" از طرف '
                . \Auth::user()->first_name . ' ' . \Auth::user()->last_name . " ارسال شده است.\n"
                . 'کد دسترسی: ' . $VerifyCode;

            if (SmsHandlerController::Send(["$engineeringManager->phone"], $MessageText)) {
                $checkAccess = UserAccessHandler::where('target_id', $id)->where('user_id', auth()->id())->first();
                if ($checkAccess) {
                    $checkAccess->delete();
                }
                UserAccessHandler::create([
                    'user_id' => auth()->id(),
                    'access_type' => 'freelancer offer',
                    'target_id' => $id,
                    'access_code' => $VerifyCode,
                    'receive_user_id' => $engineeringManager->id,
                ]);
...
//        }
            ...
            if ($request->status == 'winner') {
                ...
                if ($FreelancerOffer->workPackageTable->status != 'activated') {
                    $FreelancerOffer->workPackageTable->update([
                        'status' => 'activated',
                        'wp_final_time' => $FreelancerOffer->time,
                        'wp_final_price' => $FreelancerOffer->price,
                        'wp_activation_time' => Carbon::now(),
    ...
    public function sortUpdate(Request $request, $id)
    {
        ...
        if ($WorkPackage->offer_list_status == null) {
            $WorkPackage->update(['offer_list_sorting' => $request->winning_formula]);
