...
// همه‌ی user_id‌هایی که قرارداد نفرساعتی امضا کرده‌اند
    ...
    * */
    ...
    {
        $UserData = Users::join('freelancer', 'users.id', '=', 'freelancer.users_id')->select('first_name', 'last_name', 'email', 'phone', 'freelancer.meli_code', 'freelancer.cardserialno', 'freelancer.birthday', 'freelancer.birthday_miladi', 'freelancer.postal_code', 'freelancer.address', 'freelancer.certpass')->find(auth('sanctum')->user()->id);
//        $workPackage = WorkPackage::find($id);
        $freelancerOffer = FreelancerOffer::where('work_package_id', $id)->where('user_id', auth('sanctum')->user()->id)->where('status', 'awaiting_signature')->first();
    ...
    public function workPackageSignatureDigestAction(Request $request, $id)
    {
        ...
        if ($getData['userData']['meli_code'] && $freelancerOffer && $getData['details']->status === 'awaiting_signature') {
            ...
            if (!$ContractCheck) {
                ...
                $ContractFile = FreelancerContract::create([
    ...
    {
        $getData = [];
        $getData['userData'] = Users::join('freelancer', 'users.id', '=', 'freelancer.users_id')->select('first_name', 'last_name', 'email', 'phone', 'avatar', 'freelancer.meli_code', 'freelancer.cardserialno', 'freelancer.sarbazi', 'freelancer.sarbazi_file', 'freelancer.birthday', 'freelancer.birthday_miladi', 'freelancer.shenasnameh', 'freelancer.mahale_sodoor', 'freelancer.country', 'freelancer.home_phone', 'freelancer.postal_code', 'freelancer.address', 'freelancer.linkedin', 'freelancer.website', 'freelancer.biography', 'freelancer.certpass')->find(auth('sanctum')->user()->id);
        $workPackage = WorkPackage::select('id', 'title', 'desc', 'package_number', 'wp_final_price', 'wp_final_time', 'status', 'wp_activation_time', 'wp_activation_time', 'signature', 'daily_fine', 'fine_after_day', 'fine_after_price', 'fine_after_negative')->find($id);
        $getData['details'] = $workPackage;
        $freelancerOffer = FreelancerOffer::where('work_package_id', $id)->where('user_id', auth('sanctum')->user()->id)->where('status', 'awaiting_signature')->count();
        ...
        if ($getData['userData']['meli_code'] && $freelancerOffer && $getData['details']->status === 'awaiting_signature') {
            ...
            if ($SignProcess['errorCode'] === 0) {
                ...
                \File::makeDirectory(storage_path() . '/app/contract/signed/', $mode = 0755, true, true);
                $pdfName = $getData['userData']['meli_code'] . '-' . date('Y-m-d_H-i-s') . '.pdf';
                $pdf = fopen(storage_path('app/contract/signed/' . $pdfName), 'w');
                fwrite($pdf, base64_decode($SignPDF['result']));
                fclose($pdf);

                $FreelancerContract = FreelancerContract::where('work_package_id', $id)->where('user_id', auth('sanctum')->user()->id)->first();
                ...
                if ($workPackage->status === 'awaiting_signature') {
                    $workPackage->update(['status' => 'pre_activate', 'signature' => 'yes']);
                    FreelancerOffer::where('work_package_id', $id)->where('user_id', auth('sanctum')->user()->id)->update([
                        'status' => 'winner',
                    ]);
//                    $freelancerOffer->update(['status' => 'winner']);
                    return [
...
//            $digestData = SignatureSystemController::dss('digest', [
//                "pdfData" => base64_encode(file_get_contents('test.pdf')),
//                "signerCertificate" => (string)$data['signRequest']['certificate'],
//                "certificationLevel" => 0,
//                "dateTime" => date('c'),
//                "reason" => "تست امضا",
//                "location" => "",
//                "imageDataUrl" => (string)$data['signRequest']['SignImage'],
//                "page" => 1,
//                "lowerLeftX" => 0,
//                "lowerLeftY" => 100,
//                "upperRightX" => 200,
//                "upperRightY" => 200,
//                "signatureFieldName" => (string)$UserData['meli_code'],
//                "hashAlgorithm" => 1
//            ]);
//
//            if ($data['digest']['statusCode'] === 200) {
//                return [
//                    'signRequest' => $data,
//                    'digest' => $data['digest'],
//                ];
//
////                $data['signProcess'] = SignatureSystemController::signature('signProcess', [
////                    "signId" => $data['signRequest']['signId'],
////                    "dataforsign" => $data['digest']['result'],
