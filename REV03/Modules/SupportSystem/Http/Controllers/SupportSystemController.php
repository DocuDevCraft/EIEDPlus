...
//            }
    ...
    public function TicketReply(TicketReplyRequest $request, $id)
    {
        ...
        $TicketData['uid'] = \auth('sanctum')->user()->id;

        /* Store Attachments */
        if ($request->file('attachments')) {
            $TicketData['attachments'] = [];
            array_push($TicketData['attachments'], FileLibraryController::upload($request->file('attachments'), 'file', 'support/files', 'support'));
//            foreach ($request->file('attachments') as $item) {
...
//            }
    ...
    public function tickets_store(TicketRequest $request, $id)
    {
        ...
        if (Ticket::create($TicketData)) {
            ...
            NotificationController::store('تیکت شما پاسخ داده شد', HomeController::TruncateString($request->replay_text, 50, 50), $Support->uid);
            return redirect()->back()->with('notification', [
                'class' => 'success',
